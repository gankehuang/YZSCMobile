<template>
    <view class="mpvue-picker">
        ...
        <view class="mpvue-picker-content " :class="{'mpvue-picker-view-show':showPicker}">
			...
			<!-- 日期选择器 -->
			<picker-view indicator-style="height: 40px;" class="mpvue-picker-view" :value="pickerValue" @change="pickerChange" v-if="mode==='dateSelector'">
			    <block>
			        <picker-view-column>
			            <view class="picker-item" v-for="(item,index) in pickerValueYears" :key="index">{{item.label}}</view>
			        </picker-view-column>
			        <picker-view-column>
			            <view class="picker-item" v-for="(item,index) in pickerValueMonths" :key="index">{{item.label}}</view>
			        </picker-view-column>
					<picker-view-column>
					    <view class="picker-item" v-for="(item,index) in pickerValueDays" :key="index">{{item.label}}</view>
					</picker-view-column>
			    </block>
			</picker-view>
            <!-- 多列选择 -->
        ...
        methods: {
            ...
            _getPickerLabelAndValue(value, mode) {
                ...
                if (mode === 'selector') {
                    ...
                    pickerLable = `${this.pickerValueYears[value[0]].value}-${this.pickerValueMonths[value[1]].value}-${this.pickerValueDays[value[2]].value}`;
