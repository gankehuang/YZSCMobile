	...
	<view>
		...
		<view class="status">
			...
			<view class="status-header">
				...
				<view class="status-title">后备猪健康查询</view>
		...
		<popup-layer ref="popupRef1" :direction="'left'" :class="{ active: active }">
			<scroll-view class="draw" scroll-y="true" style="padding-top: 65px;">
				...
				<!-- 健康分类 -->
				<draw-cell title="健康分类" required="true">
					<view slot="value" class="bg-gray jus-j"><input type="text" placeholder="请输入阶段" v-model="searchForm.recyclingNum" /></view>
				</draw-cell>
			</scroll-view>
...
export default {
	...
	methods: {
		...
		rowTapHandler(row) {
			uni.navigateTo({
				url: `/pages/dataCollection/reservePig/reservePigDetail?id=${row.id}`,
				success: res => {},
				fail: () => {},
				complete: () => {}
			});
