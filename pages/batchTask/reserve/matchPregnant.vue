...
export default {
	...
	methods: {
		...
		toTask(){ // 跳转预警任务 
			uni.navigateTo({
				url: '/pages/batchTask/reserve/warningTask'
			})
		},
		toRecord(){  //跳转任务记录
			uni.navigateTo({
				url: '/pages/batchTask/reserve/taskRecord'
			})
		},
		toStartTask(item) {  //跳转开始任务
			if(item.taskName == '母猪背膘调整') {
				uni.navigateTo({
					url: '/pages/batchTask/reserve/startTask?item=' + encodeURIComponent(JSON.stringify(item))
				})
			} else if(item.taskName == '产后7天母猪健康情况') {
				uni.navigateTo({
					url: '/pages/batchTask/reserve/startTask7Day?item=' + encodeURIComponent(JSON.stringify(item))
				})
			} else if(item.taskName == '10日龄仔猪断奶重预估与处理') {
				uni.navigateTo({
					url: '/pages/batchTask/reserve/startTask10Day?item=' + encodeURIComponent(JSON.stringify(item))
				})
			} else if(item.taskName == '驯化效果评估') {
				uni.navigateTo({
					url: '/pages/batchTask/reserve/startTask4?item=' + encodeURIComponent(JSON.stringify(item))
				})
			} else if(item.taskName == '15日龄仔猪上市计划制定') {
				uni.navigateTo({
					url: '/pages/batchTask/reserve/startTask15Day?item=' + encodeURIComponent(JSON.stringify(item))
				})
			} else if(item.taskName == '淘汰母猪确定') {
				uni.navigateTo({
					url: '/pages/batchTask/reserve/startTask5?item=' + encodeURIComponent(JSON.stringify(item))
				})
			}
